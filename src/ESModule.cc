...
JSModuleDef* ESModuleDefine::init(JsEngine* engine) const {
    return JS_NewCModule(engine->context_, name_.c_str(), [](JSContext* ctx, JSModuleDef* module) -> int {
        ...
        for (auto& c : def->class_) {
            ...
            int  code = JS_AddModuleExport(ctx, module, c->name_.c_str());
            JsException::check(code);
            code = JS_SetModuleExport(ctx, module, c->name_.c_str(), JS_DupValue(ctx, ctor));
            JsException::check(code);
        }
